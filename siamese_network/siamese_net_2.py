...
def indices_save(dataset):
    ...
    class_map = {}
    for index, cancer_type in dataset['type_attack'].items():
        if cancer_type in class_map:
            class_map[cancer_type].append(index)
        else:
            class_map[cancer_type] = [index]
    return class_map
...
def siamese_network(risultati_siamese, dataset, input_shape, features_len, attack_type):
    ...
    x_train, x_test = train_test_split(dataset, test_size=0.20, stratify=attack_type, random_state=42)
