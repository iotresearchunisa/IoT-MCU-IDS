...
class SiameseNet:
    def __init__(self, input_shape, verbose=True):
        ...
        lr = 0.0001 # provare con 0.000005
        self.convnet.add(
        ...
        #######################################################################################
        # Ulterior 2 layers
        self.convnet.add(Conv1D(filters=64, kernel_size=3, strides=1, activation='sigmoid',
                                 padding='same'))
        self.convnet.add(MaxPooling1D(pool_size=2))

        self.convnet.add(Conv1D(filters=32, kernel_size=3, strides=1, activation='sigmoid',
                                 padding='same'))
        self.convnet.add(MaxPooling1D(pool_size=2))
        #######################################################################################
