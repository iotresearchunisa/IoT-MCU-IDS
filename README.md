...
<br><br>
Another important objective of this thesis is to design and test a Siamese neural network as an Intrusion Detection System (IDS), which can recognise most known attacks in IoT networks. The network was tested in both environments, with and without MQTT traffic generated by IoT devices to understand its adaptability and robustness. The Siamese network was also trained using the TON\_IoT dataset to demonstrate that the dataset created offers comparable, if not superior, performance compared to an already established dataset in the literature. This comparison highlights the validity of the dataset created and its potential use in academic and research contexts, as it provides equally reliable results for the classification and detection of anomalies in IoT systems. At the end, the trained Siamese network model is deployed on an ESP32 embedded system for practical effectiveness. This included bringing it under attack repeatedly by testing its response times for effective attack detection and response in real-life scenarios with the objective of achieving a lightweight yet reliable IDS suitable for IoT.
